...
urlpatterns = [
    ...
    path('logout/', auth_views.LogoutView.as_view(next_page='login'), name='logout'),  # Оставляем только один путь для логаута
    ...
    path('create-superuser/', views.create_superuser, name='create_superuser'),  # Путь для создания суперпользователя
]
